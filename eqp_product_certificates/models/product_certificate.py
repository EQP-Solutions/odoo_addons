...
class ProductCertificate(models.Model):
    ...
    description = fields.Text(
        ...
        required=True,
        help="Add a brief description of the " "certificate",
    )
    ...
    product_id = fields.Many2one(
        ...
        check_company=True,
        ondelete="restrict",
        help="Select the product that this certificate applies.",
    )
    ...
    start_date = fields.Date(
        ...
        tracking=True,
        help="The issue date of the certificate.",
    )
    ...
    certificate_file = fields.Binary(
        string="Certificate (PDF) File", help="PDF file which contain the certificate.."
    )
    ...
    state = fields.Selection(
        [("pre-active", "Pre-Active"), ("active", "Active"), ("expired", "Expired")],
        string="Status",
        compute="_compute_state",
        search="_state_search",
        readonly=True,
    )
    stage_required = fields.Boolean(
        string="Required Stage",
        readonly=True,
        related="company_id.product_certificate_stage_required",
    )
    stage_id = fields.Many2one(
        "product.certificate.stage",
        string="Stage",
        index=True,
        tracking=True,
        ondelete="restrict",
    ...
    tag_ids = fields.Many2many(
        ...
        "product_certificate_tag_tag_rel",
        "certificate_id",
        "tag_id",
        string="Tags",
        tracking=True,
    ...
    def check_search_operators(operator, value):
        ...
        if (
            (value is False and operator == "!=")
            or (value is True and operator == "=")
            or (value == [] and operator == "not in")
        ):
            return expression.TRUE_DOMAIN
        elif (value is False and operator == "=") or (
            value is True and operator == "!="
        ):
    ...
    @api.depends(
        "expiration_date",
        "company_id.product_certificate_days_to_red",
    ...
    def _compute_traffic_light(self):
        ...
        for certificate in self:
            ...
            certificate.traffic_light = (
                "red"
                if days_to_expire <= days_to_red
                else "yellow" if days_to_expire <= days_to_yellow else "green"
            )
    ...
    def _traffic_light_search(self, operator, value):
        ...
        if operator == "=":
            ...
            if (value == ["red"] and operator == "in") or (
                "green" and "yellow" in value and operator == "not in"
            ):
                max_days = days_to_red
            elif ("red" and "yellow" in value and operator == "in") or (
                value == ["green"] and operator == "not in"
            ):
                max_days = days_to_yellow
            elif (value == ["yellow"] and operator == "in") or (
                "green" and "red" in value and operator == "not in"
            ):
                min_days = days_to_red
                max_days = days_to_yellow
            elif (value == ["green"] and operator == "in") or (
                "red" and "yellow" in value and operator == "not in"
            ):
                min_days = days_to_yellow
            elif ("green" and "yellow" in value and operator == "in") or (
                value == ["red"] and operator == "not in"
            ):
                min_days = days_to_red
            elif ("green" and "red" in value and operator == "in") or (
                value == ["yellow"] and operator == "not in"
            ):
                max_days = days_to_red
                min_days = days_to_yellow
                or_operator = True
...
class ProductCertificateTag(models.Model):
    ...
    name = fields.Char(
        string="Tag Name", required=True, tracking=True, help="Name of the Tag"
    )
    company_id = fields.Many2one(
        "res.company",
        string="Company",
    ...
    description = fields.Text(
        ...
        required=True,
        help="Add a brief description of the Tag.",
    )
    color = fields.Integer(
        "Color",
        default=0,
        help="Select a color to visually associate with for quick and easy"
        " identification.",
    )

    _sql_constraints = [
        ("name_unique", "unique(name, company_id)", "A unique Tag per company.")
    ]
...
class ProductCertificateStage(models.Model):
    ...
    name = fields.Char(
        string="Stage Name", required=True, tracking=True, help="Name of the Stage"
    )
    ...
    description = fields.Text(
        string="Stage description", help="Add a brief description of the Stage."
    )
    color = fields.Integer(
        "Color",
        default=0,
    ...
    _sql_constraints = [
        ("name_unique", "unique(name, company_id)", "A unique Stage Name per company."),
        (
            "sequence_unique",
