...
class ResCompany(models.Model):
    ...
    @api.constrains(
        "product_certificate_days_to_red", "product_certificate_days_to_yellow"
    )
    def _check_valid_certificate_days(self):
        for company in self:
            if (
                company.product_certificate_days_to_yellow
                <= company.product_certificate_days_to_red
            ):
                raise ValidationError(
                    "The Yellow Days must be greater than the Red Days."
                )
